...
The variable `this` is the state object of the automation at the moment of triggering the actions. State objects also contain context data which can be used to identify the user that caused a {% term script %} or {% term automation %} to execute. Note that `this` will not change while executing the {% term actions %}.
...
| `trigger.details` | Object with matched slot details by name, such as wildcards. Each detail contains: `name` - name of the slot`text` - matched text`value` - output value (see lists)
