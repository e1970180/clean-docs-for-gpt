...
- Upgrade instructions using Linux)
- Upgrade instructions using Windows (Aeotec)
- Upgrade instructions using Windows (Zooz)
- Upgrade instructions using Windows/Linux (Z-Wave.Me)
...
The Aeotec Z-Stick and some of its variants (e.g. Z-Wave.Me UZB1) are known to have compatibility issues with the Linux kernel because of their non-compliant behavior. Plugging these controllers through a USB hub can serve as a workaround that sometimes mitigates the issue.
