...
- You can verify your configuration's {% term YAML %} structure using this online YAML parser or YAML Validator.
- To learn more about the quirks of {% term YAML %}, read YAML IDIOSYNCRASIES by SaltStack (the examples there are specific to SaltStack, but do explain YAML issues well).
...
Another common problem is that a required configuration setting is missing. If this is the case, the integration will report this to `home-assistant.log`. You can have a look at the various integration pages for instructions on how to setup the integrations.
...
4. To see the error in the logs, you need to reproduce the error. Continue with the steps on disabling debug logging and download logs.
